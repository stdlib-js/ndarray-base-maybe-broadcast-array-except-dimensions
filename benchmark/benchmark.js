...
bench( format( '%s::base_ndarray,2d', pkg ), function benchmark( b ) {
	...
	for ( i = 0; i < b.iterations; i++ ) {
		out = maybeBroadcastArrayExceptDimensions( values[ i%values.length ], [ 2, 2, 2 ], [ -1 ] ); // eslint-disable-line max-len
...
bench( format( '%s::base_ndarray,2d,same_shape', pkg ), function benchmark( b ) {
	...
	b.tic();
	for ( i = 0; i < b.iterations; i++ ) {
		out = maybeBroadcastArrayExceptDimensions( values[ i%values.length ], shape, [ -1 ] ); // eslint-disable-line max-len
		if ( typeof out !== 'object' ) {
			b.fail( 'should return an object' );
		}
	}
